        ...
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <main className="flex-grow container mx-auto px-4 py-8 mt-24">
                ...
                <Row gutter={[24, 24]} className="mb-8">
                    ...
                    <Col xs={24} sm={6}>
                        <Card hoverable className="rounded-xl shadow-sm border-purple-100">
                            <Statistic
                ...
                <Modal
                    title="Parking Space Application Details"
                        ...
                        <Tabs defaultActiveKey="details">
                            <Tabs.TabPane tab="Space Details" key="details">
                                <div className="space-y-4 pt-4">
                                    <div>
                                        ...
                                        <p className="font-semibold text-lg">{selectedSpace.location?.address}</p>
                                    </div>
                                    <Row gutter={16}>
                                        <Col span={12}>
                                            <Text type="secondary">Total Capacity:</Text>
                                            <p className="font-semibold">{selectedSpace.capacity} Vehicle(s)</p>
                                        </Col>
                                        <Col span={12}>
                                            <Text type="secondary">Pricing Model:</Text>
                                            <p className="font-semibold">{selectedSpace.dynamicPricing?.isDynamic ? 'Dynamic Enabled' : 'Static Flat Rate'}</p>
                                            {selectedSpace.dynamicPricing?.isDynamic && (
                                                <div className="mt-1 text-sm bg-purple-50 p-2 rounded border border-purple-100">
                                                    <p className="m-0"><span className="text-gray-500">Peak Hours: </span>{selectedSpace.dynamicPricing.peakStartTime} - {selectedSpace.dynamicPricing.peakEndTime}</p>
                                                    <p className="m-0"><span className="text-gray-500">Peak Multiplier: </span><span className="font-bold text-purple-600">x{selectedSpace.dynamicPricing.peakMultiplier}</span></p>
                                                </div>
                                            )}
                                        </Col>
                                    </Row>
                                    <Row gutter={16}>
                                        <Col span={12}>
                                            ...
                                            <p className="font-semibold text-blue-600">${selectedSpace.rates?.hourly}/hr</p>

                                            {selectedSpace.rates?.customTiers?.length > 0 && (
                                                <div className="mt-2 text-sm">
                                                    <Text type="secondary">Long-Term Discount Tiers:</Text>
                                                    <ul className="list-disc pl-4 m-0 text-green-700 font-medium">
                                        ...
                                        <Col span={12}>
                                            ...
                                            <p className="font-semibold text-blue-600">${selectedSpace.rates?.daily}/day</p>
                                        </Col>
                                    </Row>
                                    <div>
                                        <Text type="secondary">Rules & Regulations defined by Owner:</Text>
                                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 mt-2">
                                            {(() => {
                                                const rules = selectedSpace.rules;
                                                if (!rules || rules.length === 0) {
                                                    return <Text type="secondary" className="italic">No specific rules provided</Text>;
                                                }
                                                ...
                                                return (
                                                    <ul className="list-disc pl-4 m-0 text-sm text-gray-700">
                            ...
                            <Tabs.TabPane tab="Discussion" key="chat">
                                <div className="flex flex-col h-[400px]">
                                    <div className="flex-grow overflow-y-auto bg-gray-50 border border-gray-200 rounded p-4 mb-4">
                                        {chatLoading ? (
                                            ...
                                            messages.map(msg => {
                                                ...
                                                return (
                                                    <div key={msg._id} className={`mb-4 flex flex-col ${isAdminMode ? 'items-end' : 'items-start'}`}>
                                                        ...
                                                        <div className={`px-4 py-2 rounded-lg max-w-[85%] ${isAdminMode ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white border border-gray-200 text-gray-800 rounded-bl-none shadow-sm'}`}>
                                                            {msg.message}
                                                        </div>
                                                    </div>
                                                );
                                    ...
                                    <div className="flex gap-2">
                                        <textarea
                                            className="w-full flex-grow p-2 border border-gray-300 rounded focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none resize-none"
                                            rows="2"
